...
$longJobScriptBlock = {
    Start-Sleep -s 5 #We use this command to simulate a command that takes a while to run. Wait for 10 seconds.
    Get-ChildItem C:\
}
...
#$EventArgs
$Event
    Write-Host "`n Long job state is $($Sender.State)!" -ForegroundColor Green

    #Step 4: Capture the result of the job to a new variable.
    $longJobResult = Receive-Job $longJob
